...
import ky from "ky";
import { TCalendlyAvailableSlot, TCalendlyEventTypeAvailableTimesGetResponse, TRawCalendlyAvailableSlot } from "types/calendly";
...
export async function getCalendlyDaysAvailableSlots(days: number): Promise<TCalendlyAvailableSlot[]> {
    ...
    // const calendlyAvailableSlots = await Promise.all(Array.from({ length: weeks }).map(async (_, i) => {
    ...
    //     const calendlyData = await kyCalendly.get<TCalendlyEventTypeAvailableTimesGetResponse>('https://api.calendly.com/event_type_available_times', {
    //         searchParams,
    //     }).json();
