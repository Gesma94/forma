...
export function InlineGalleryCarousel({ module, imagesUrl }: TProps) {
  ...
  useLayoutEffect(() => {
    ...
    middleElement.scrollIntoView({ inline: 'center' });
  }, []);
  ...
  return (
    <div className='relative mt-10'>
      ...
      <div
        ...
        className='flex gap-10 h-[720px] overflow-auto relative px-10 sm:px-20 lg:px-40 snap-x snap-mandatory touch-pan-x'
        style={{ scrollbarWidth: 'none' }}
      >
        {imagesUrl.map((imageUrl, i) => {
          const image = module.images[i];
          return (
            <div key={image._key} className='h-full w-auto shrink-0 relative max-w-[calc(100vw-4rem)] snap-center'>
